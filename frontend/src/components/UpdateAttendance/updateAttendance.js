...
function UpdateAttendance() {
    ...
    useEffect(() => {
        axios
            .post("http://localhost:4000/teacher/getLeaves", values)
            .then((res) => {
                if (res.data.Status === "Success") {
                    // console.log(res.data.data, "leave data");   
            ...
            let obj = updatedArray.find((o, i) => {
                if (o.student_id === student_id && o.date === current) {
                    ...
                    if (t !== "2") {
                        ...
                        const ele = document.getElementById(`${student_id}-${TODAY}`);
    ...
    function ShowLeave(props) {
        ...
        // today : TODAY
        var updateLeave = leaveData.find((obj) => {
            if (obj.student_id === sid && (TODAY >= obj.date_from && TODAY <= obj.date_to)) {
                return true;
            }
        ...
        useEffect(()=> {
            if(updateLeave)
                axios
                    .post("http://localhost:4000/teacher/enterLeaveAttendance", values5)
                    .then((res) => {
                        if(res.data.Status === "Success"){
                            // alert('Record Updated Successfully');
                            console.log("Show leave");
                        } else {
                            setError5(res.data.Error);
                        }
                    })
                    .catch((err) => console.log(err));
        }, []);
                    ...
                    <MDBTable className="table-attendance-update ">
                        ...
                        <MDBTableHead>
                            <tr>
                                ...
                                {dateArray.map((val, key) => {
                                    ...
                                    return (
                                        <th className="updateAttendance-th" key={key}>
                                            {date[8] + date[9] + " " + mon}
                                        </th>
                                    );
                                })}
                        ...
                        <tbody>
                            {studentData.map((val, key) => {
                                ...
                                return (
                                    <tr key={student_id}>
                                        <td className="updateAttendance-td">{key + 1}.</td>
                                        <td className="updateAttendance-td">{student_id}</td>
                                        <td className="updateAttendance-td">
                                            {val.first_name} {val.last_name}
                                        </td>
                                        {dateArray.map((val, key) => {
                                            return (
                                                <td className="updateAttendance-td" key={key}>
                                                    <ChecklistItem
                                                        val={val}
                                                        key={key}
                                                        student_id={student_id}
                            ...
                            <tr id="totPresent">
                                ...
                                <td className="totPresent1 updateAttendance-td">Total</td>
                                <td className="totPresent2 updateAttendance-td">Present :</td>
                                {dateArray.map((val, key) => {
                                    return <td className="totPresentCount" key={key}>{val.totPresent}</td>;
                                })}
                            </tr>
                        </tbody>
                    </MDBTable>

                    <button id="attendanceEditButton" onClick={handleEditButton}>
                        {buttonText}
                    </button>

                    <button id="attendanceSubmitButton" onClick={handleSubmit}>
                        Submit
                    </button>
